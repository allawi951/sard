...
function start() {
  ...
  /* ── ٧) ترتيب المُشغّلات بترتيب الصفحة لا بترتيب الكود ──

     المُشغّلات تُنشأ هنا بترتيب ثابت (الطبقات ثم المجموعة)، بينما التاجر
     يرتّب المكوّنات كما يشاء من لوحة التحكم. فإن قدّم «المجموعة» على
     «الطبقات» صار ترتيب الإنشاء مخالفًا لترتيب المستند، فتُحسب بداية القسم
     اللاحق **قبل** أن يُدرِج القسم السابق مسافة تثبيته — فيبدأ تثبيته مبكّرًا
     ويعلو القسم الجاري. (مقيس: بداية الطبقات سبقت موضعها بـ 1336px — وهي
     مسافة تثبيت المجموعة بالضبط — فتراكبا 436px.)

     `sort()` يعيد ترتيب المصفوفة الداخلية بترتيب الظهور في الصفحة فتُحسب
     الحدود بالتتابع الصحيح. ونعيده عند كل تحديث (تغيير مقاس، اكتمال صور)
     لأن الترتيب يجب أن يبقى صحيحًا لا أن يُضبط مرة واحدة. */
